<script setup lang="ts">
  ...
  import {accessPolicyPatch, accessPolicyDelete} from '~/composables/ApiClient'
  ...
  const updating_policy: Ref<boolean> = ref(false)
  const deleting_policy: Ref<boolean> = ref(false)

  const resp: Resp = await accessPolicyGet(enterprise_id, policy_id)

  if (resp.status === 200) {
    const policyFromApi = resp.json.policy
    policyData.value = {
      applications: policyFromApi.applications,
      passwordPolicies: policyFromApi.passwordPolicies,
      cameraAccess: policyFromApi.cameraAccess,
      microphoneAccess: policyFromApi.microphoneAccess,
      systemUpdate: {
        type: policyFromApi.systemUpdate.type ?? 'SYSTEM_UPDATE_TYPE_UNSPECIFIED'
      },
      screenCaptureDisabled: policyFromApi.screenCaptureDisabled ?? false,
      smsDisabled: policyFromApi.smsDisabled ?? false,
      bluetoothDisabled: policyFromApi.bluetoothDisabled ?? false,
      factoryResetDisabled: policyFromApi.factoryResetDisabled ?? false,
      modifyAccountsDisabled: policyFromApi.modifyAccountsDisabled ?? false,
      autoDateAndTimeZone: policyFromApi.autoDateAndTimeZone,
      deviceConnectivityManagement: {
        usbDataAccess: policyFromApi.deviceConnectivityManagement.usbDataAccess,
        tetheringSettings: policyFromApi.deviceConnectivityManagement.tetheringSettings,
        wifiSsidPolicy: {
          wifiSsidPolicyType: policyFromApi.deviceConnectivityManagement.wifiSsidPolicy.wifiSsidPolicyType,
          wifiSsids: policyFromApi.deviceConnectivityManagement.wifiSsidPolicy.wifiSsids ?? []
        }
      },
      advancedSecurityOverrides: policyFromApi.advancedSecurityOverrides
    }
  } else {
    alert.value = {
  ...
  async function tryDeletePolicy() {
    if (confirm('本当にポリシーを削除しますか？')) {
      deleting_policy.value = true
      const resp: Resp = await accessPolicyDelete(enterprise_id, policy_id)
      if (resp.status === 200) {
        useRouter().push('/enterprise/' + enterprise_id)
      }
      else {
        deleting_policy.value = false
      }
    }
  }
</script>
...
<template>
  ...
  <div class="col-sm-9 col-md-7 col-lg-5 border border-primary bg-light p-3">
    ...
    <button type="submit" class="btn btn-primary me-3"
      @click="tryUpdatePolicy" :disabled="invalid || updating_policy"
    >
      <LoadingSpinner
        v-if="updating_policy" :size="'25'" :color="'white'"
      />
      <span v-else>更新</span>
    </button>
    <button
      type="button" class="btn btn-danger"
      @click.prevent="tryDeletePolicy" :disabled="deleting_policy"
    >
      <LoadingSpinner
        v-if="deleting_policy" class="mx-4" :size="'25'" :color="'white'"
      />
      <span v-else>ポリシーを削除</span>
    </button>
  </div>
</template>
